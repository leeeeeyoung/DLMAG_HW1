"""
DL Artist Classification Training Pipeline

This script handles the training of the artist classification model using ResNet50
backbone adapted for audio spectrograms. It includes comprehensive logging and
...
from DL_utils import (
    Config, AudioDataset, ResNetModel, LabelSmoothingLoss,
    load_paths_from_json, label_from_filepath, get_cpu_info, DEVICE
)
...
class Trainer:
    ...
    def train_one_epoch(self, epoch_idx: int) -> float:
        ...
        loop = tqdm(
            self.train_loader, 
            desc=f"Train Epoch {epoch_idx}/{cfg.EPOCHS}", 
            leave=False
        )
    ...
    def evaluate(self):
        ...
        y_score = tuple(tuple(pred) for pred_set in all_predictions for pred in pred_set)
    ...
    def get_confusion_matrix(self):
        ...
        cm = confusion_matrix(all_targets, all_predictions, labels=list(range(len(self.classes))))
    ...
    def save_confusion_matrix(self, cm, targets, predictions):
        ...
        # Set labels
        ax.set(xticks=list(range(len(self.classes))),
    ...
    def fit(self):
        ...
        logger.log_training_start({
            ...
            "Features": "Comprehensive (log mel + MFCC + spectral contrast + RMS energy)",
            "Input Channels": 11
        })
...
def main():
    ...
    try:
        ...
        classes = sorted(list({
            label_from_filepath(path) for path in train_paths
        }))
        
        logger.info(f"Loaded {len(train_paths)} training files and {len(val_paths)} validation files")
    ...
    model = ResNetModel(num_classes=len(classes), input_channels=11)  # 11 channels for comprehensive features
    ...
    confusion_matrix_path, cm_data_path = trainer.save_confusion_matrix(cm, targets, predictions)
