...
class InferenceEngine:
    ...
    def __init__(self, model_path: str, classes: List[str]):
        ...
        self.model = ResNetModel(num_classes=len(classes)).to(DEVICE)
    ...
    def predict_batch(self, file_paths: List[str]) -> Dict[str, List[str]]:
        ...
        for i, file_path in enumerate(tqdm(file_paths, desc="Processing audio files")):
            try:
                ...
                # Get predictions
                top3_predictions = predict_top3_for_audio(
                    self.model, file_path, self.classes
                )
        ...
        logger.log_inference_complete(
            cfg.TEST_OUT, successful_predictions, len(file_paths)
        )
        return predictions
...
def load_test_files() -> List[str]:
    ...
    if len(existing_files) != len(test_files):
        logger.warning(
            f"Some test files are missing: {len(test_files) - len(existing_files)} files"
        )
...
def extract_classes_from_json():
    ...
    try:
        if cfg.TRAIN_JSON.exists():
            ...
            raise FileNotFoundError(
                "Cannot find train.json or val.json for class extraction"
            )
...
def main():
    ...
    for artist, count in sorted(
        prediction_counts.items(), key=lambda x: x[1], reverse=True
    ):
